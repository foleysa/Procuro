  ...
  return (
    <div className="p-8 space-y-6 max-w-7xl">
          ...
          <CardContent>
            {topOpenOpps.length === 0 ? (
              <div className="text-sm text-muted-foreground">
                No open opportunities. Run a new OODA cycle to surface fresh ones.
              </div>
            ) : (
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0">
          <CardTitle className="flex items-center gap-2">
            <Activity className="w-5 h-5" /> Recent OODA cycles
          </CardTitle>
          <Link href="/ooda">
            <Button variant="ghost" size="sm" className="gap-1">
              Open OODA Wheel <ArrowRight className="w-3 h-3" />
            </Button>
          </Link>
        </CardHeader>
        <CardContent>
          {cycles.length === 0 ? (
            <div className="text-sm text-muted-foreground">
              No cycles yet. Trigger one from the OODA Wheel to start.
            </div>
          ) : (
...
function buildAttentionItems(args: {
  ...
  if (args.daysSinceLastCycle === null) {
    items.push({
      id: "no-cycle",
      severity: "warn",
      title: "No OODA cycle has been run yet",
      detail: "Trigger your first cycle to start surfacing opportunities",
      cta: "Run cycle",
      href: "/ooda",
    });
  } else if (args.daysSinceLastCycle >= 7) {
    items.push({
      id: "stale-cycle",
      severity: "warn",
      title: `Last OODA cycle was ${args.daysSinceLastCycle} days ago`,
      detail: "Run a fresh cycle to catch new opportunities and refresh priors",
      cta: "Run cycle",
      href: "/ooda",
    });
  }
